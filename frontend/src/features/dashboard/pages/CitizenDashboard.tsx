...
export default function CitizenDashboard() {
  ...
  const [showCriticalAlert, setShowCriticalAlert] = useState(!scenario.enabled);
  const [browserLocation, setBrowserLocation] = useState<BrowserLocation>(scenario.enabled ? scenario.origin : null);
  const [locationStatus, setLocationStatus] = useState(scenario.enabled ? 'Demo exercise location · synthetic' : 'Location not shared · exploring Houston');
  const [placeLabel, setPlaceLabel] = useState<PlaceLabel>(scenario.enabled ? { primary: 'Demo Riverside district', secondary: 'Scripted exercise location' } : { primary: 'Houston area', secondary: 'Map preview · location not shared' });
  ...
  const displayName = useMemo(() => {
    ...
    return scenario.enabled ? name || 'Demo Citizen' : name && name !== 'Demo Citizen' ? name : 'Ramesh K.';
  }, [session?.user.name]);
  ...
  const useMyLocation = () => {
    if (scenario.enabled && scenario.origin) {
      setBrowserLocation(scenario.origin); setLocationStatus('Demo exercise location · synthetic');
      void loadRoute(scenario.origin.latitude, scenario.origin.longitude); return;
    }
