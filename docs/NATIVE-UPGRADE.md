...
The expanded app passed a signed iOS Simulator build on the macOS GitHub Actions runner: [build result](https://github.com/ayushkh-txst/waysignal/actions/runs/34912217458). The earlier starter app also compiled and ran on the user's Mac. The expanded screens have not yet been visually exercised there. Simulator microphone, MapKit rendering, keyboard layout and new-screen interactions still require that run.
